...
const GRAPH_SCHEMA_VERSION = (window.UnfinishedKnowledgeGraph && window.UnfinishedKnowledgeGraph.GRAPH_SCHEMA_VERSION)
...
function buildKnowledgeGraphSnapshot(milestones) {
  ...
  milestones.forEach((item) => {
    ...
    addGraphEdge(edgeMap, {
      ...
      relationBasis: [item.field === bestMatch.field ? "field" : "", item.status === bestMatch.status ? "status" : ""].filter(Boolean)
    });
  });
...
const knowledgeGraphRuntime = window.UnfinishedKnowledgeGraph || {
  GRAPH_SCHEMA_VERSION,
  buildKnowledgeGraphSnapshot,
  createKnowledgeGraphStore(options) {
...
window.UnfinishedKnowledgeGraph = knowledgeGraphRuntime;

const knowledgeGraphStore = knowledgeGraphRuntime.createKnowledgeGraphStore({
  getMilestones: () => milestoneStore.all()
});
...
function buildStarRoadGraph(milestones, width, height) {
  const knowledgeGraph = knowledgeGraphStore.snapshot(milestones);
  ...
  knowledgeGraph.edges
    .filter((edge) => edge.type === "RELATED_TO")
    .forEach((edge) => {
      const fromProject = knowledgeGraph.nodeById.get(edge.from);
      const toProject = knowledgeGraph.nodeById.get(edge.to);
      if (!fromProject?.sourceId || !toProject?.sourceId) return;
      const visualEdgeId = [fromProject.sourceId, toProject.sourceId].sort().join("::");
      if (edgeKeys.has(visualEdgeId)) return;
      edgeKeys.add(visualEdgeId);
      edges.push({
        from: `milestone-${fromProject.sourceId}`,
        to: `milestone-${toProject.sourceId}`,
        kind: "relation",
        color: statusMeta(toProject.status).color,
        opacity: 0.24 + Math.min(Number(edge.weight) || 0, 1) * 0.2,
        width: 0.9 + Math.min(Number(edge.weight) || 0, 1) * 0.5
      });
    });
...
function renderMilestone(id) {
  ...
  const graphBundle = knowledgeGraphStore.projectBundle(id);
  ...
  const graphSummaryHtml = `
    <section class="milestone-block">
      ...
      <dl class="detail-grid">
        <div class="detail-row"><dt>失败原因节点</dt><dd>${graphBundle.reasons.map((node) => escapeHtml(node.label || node.summary)).join(" / ") || "待补充"}</dd></div>
        <div class="detail-row"><dt>可继承资产节点</dt><dd>${graphBundle.assets.map((node) => escapeHtml(node.title)).join(" / ") || "待补充"}</dd></div>
        <div class="detail-row"><dt>后续路线节点</dt><dd>${graphBundle.routes.map((node) => escapeHtml(node.title)).join(" / ") || "待补充"}</dd></div>
        <div class="detail-row"><dt>关联项目</dt><dd>${graphBundle.relatedProjects.map((node) => escapeHtml(node.title)).join(" / ") || "待补充"}</dd></div>
      </dl>
    </section>
  `;
...
function exportSubmissionsJSON() {
  const knowledgeGraph = knowledgeGraphStore.snapshot();
  const data = {
    ...
    graph_snapshot: {
      ...
      exportedAt: knowledgeGraph.exportedAt,
